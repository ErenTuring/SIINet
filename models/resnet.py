...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, output_stride=16):
        ...
        for i in range(int(math.log2(output_stride))-2):
            s[i] = 2
        self.layer1 = self._make_layer(block, 64, layers[0], stride=1)  # s/4
        self.layer2 = self._make_layer(block, 128, layers[1], stride=s[0])  # s/out_s
        self.layer3 = self._make_layer(block, 256, layers[2], stride=s[1])
        self.layer4 = self._make_layer(block, 512, layers[3], stride=s[2])
